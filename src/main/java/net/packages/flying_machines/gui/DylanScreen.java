...
public class DylanScreen extends HandledScreen<DylanScreenHandler> {
    ...
    protected void init() {
        ...
        this.button1 = this.addDrawableChild(
                    ButtonWidget.builder(Text.translatable("button.make_a_gift"), button -> {
                        handleButtonPress();
                    }).dimensions(this.width / 2 - 230, this.height / 2 - 18, 120, 20).build()
        );
        this.addDrawableChild(
                ButtonWidget.builder(Text.translatable("button.option2"), button -> {
                }).dimensions(this.width / 2 - 230, this.height / 2 + 3, 120, 20).build()
        );
        updateButtonState();
    }
